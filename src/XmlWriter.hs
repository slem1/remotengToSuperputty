...
renderConnections :: [Maybe Connection] -> BSL.ByteString
renderConnections cnxs = xrender ( buildDoc cnxs )

buildDoc :: [Maybe Connection] -> Xml Doc
...
connectionToSessionData :: Maybe Connection -> Xml Elem
connectionToSessionData Nothing = xempty
connectionToSessionData (Just c) = xelem (T.pack "SessionData") $ xattrs [
